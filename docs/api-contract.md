...
- Browser resource paths are explicit: `/api/organizations/:organizationId/*`.
  ...
  Session APIs currently satisfy this boundary. Resource classes without durable
  organization ownership are deliberately absent from the scoped router rather
  than exposed through an authorization-only alias.
