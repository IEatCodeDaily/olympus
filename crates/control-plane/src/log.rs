...
fn clear_all(tx: &Transaction<'_>) -> Result<()> {
    tx.execute_batch("DELETE FROM events; DELETE FROM sessions; DELETE FROM messages; DELETE FROM cards; DELETE FROM setup; DELETE FROM registry; DELETE FROM projects; DELETE FROM repos; DELETE FROM session_repos; DELETE FROM sqlite_sequence WHERE name='events';")?;
    Ok(())
}
...
fn event_session_id(event: &Event) -> Option<&str> {
    match event {
        ...
        Event::SessionForked { child_session_id, .. } => Some(child_session_id),
        Event::SessionHandover { target_session_id, .. } => Some(target_session_id),
