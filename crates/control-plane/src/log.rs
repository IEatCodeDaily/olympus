...
impl Log {
    ...
    /// Import the old redb log once. Existing SQLite events make this a no-op.
    pub fn migrate_from_redb(&self, path: &Path) -> Result<usize> {
        ...
        let mut migrated = 0;
        loop {
            let page = legacy.read_from(next, 1_000)?;
            if page.is_empty() {
                break;
            }
...
fn append_in_tx(tx: &Transaction<'_>, event: &Event) -> Result<u64> {
    ...
    tx.execute(
        ...
        params![event_type(event), payload, event_time(event)],
    )?;
...
CREATE TABLE IF NOT EXISTS events(seq INTEGER PRIMARY KEY AUTOINCREMENT,event_type TEXT NOT NULL,payload BLOB NOT NULL,created_at REAL NOT NULL);
