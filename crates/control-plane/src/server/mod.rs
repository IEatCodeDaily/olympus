...
async fn organization_resource_proxy(
    ...
    let Some(suffix) = original
        ...
        .and_then(|path| path.split_once('/').map(|(_, resource)| resource))
    else {
    ...
    *request.headers_mut() = parts.headers;
    request
        .extensions_mut()
        .insert(OrganizationScope(organization_id));
...
mod tests {
    ...
    async fn login_cookie_authenticates_session_and_organization_routes() {
        ...
        let unsupported_global_resource = app
            .clone()
            .oneshot(
                Request::builder()
            ...
            .await
            .unwrap();
        assert_eq!(unsupported_global_resource.status(), StatusCode::NOT_FOUND);
